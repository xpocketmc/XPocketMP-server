...
- Added a new cache mechanism for `xpocketmp-MP.phar`. This has several advantages:
...
- The following API methods have been added:
  - `public ArmorMaterial->getEquipSound() : ?\xpocketmp\world\Sound` - returns the sound to play when this armour is equipped or unequipped
...
## Dependencies
- Now uses [`xpocketmp/bedrock-block-upgrade-schema` version 4.0.0](https://github.com/pmmp/BedrockBlockUpgradeSchema/releases/tag/4.0.0).
- Now uses [`pmmp/ext-pmmpthread` version 6.1.0](https://github.com/pmmp/ext-pmmpthread/releases/tag/6.1.0).
- Now uses [`xpocketmp/errorhandler` version 0.7.0](https://github.com/pmmp/ErrorHandler/releases/tag/0.7.0).
- Now uses [`xpocketmp/raklib` version 1.1.0](https://github.com/pmmp/RakLib/releases/tag/1.1.0).
- Now uses [`xpocketmp/raklib-ipc` version 1.0.0](https://github.com/pmmp/RakLibIpc/releases/tag/1.0.0).
...
- Added a new cache mechanism for `xpocketmp-MP.phar`.
  ...
  - The phar stub (`build/server-phar-stub.php`) now converts the phar contents into a `.tar`, and decompresses all the files into `$TMPDIR/xpocketmp-MP-phar-cache.<random>/`.
  - `phar://` URIs still work with this system, but `new Phar(__FILE__)` must be replaced by `new PharData(__FILE__)` within xpocketmp-MP core code.
