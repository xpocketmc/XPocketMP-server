...
### Dependency changes
- The `xpocketmp_chunkutils` PHP extension has been dropped.
...
- The following Composer dependency versions have changed (**PLEASE READ, dependency API changes are not mentioned in this changelog!**):
  - `xpocketmp/bedrock-protocol` has been added at [7.0.0](https://github.com/pmmp/BedrockProtocol/releases/tag/7.0.0+bedrock-1.18.0).
  - `xpocketmp/classloader` has been updated from 0.1.0 to [0.2.0](https://github.com/pmmp/ClassLoader/releases/tag/0.2.0) (**significant API changes, new features**).
  - `xpocketmp/color` has been added at [0.2.0](https://github.com/pmmp/Color/releases/tag/0.2.0).
  - `xpocketmp/errorhandler` has been added at [0.3.0](https://github.com/pmmp/ErrorHandler/releases/tag/0.3.0).
  - `xpocketmp/locale-data` has been added at [2.0.16](https://github.com/pmmp/Language/releases/tag/2.0.16).
  - `xpocketmp/log` has been updated from 0.2.0 to [0.4.0](https://github.com/pmmp/Log/releases/tag/0.4.0) (**minor API changes**, see also [0.3.0](https://github.com/pmmp/Log/releases/tag/0.3.0)).
  - `xpocketmp/nbt` has been updated from 0.2.18 to [0.3.0](https://github.com/pmmp/NBT/releases/tag/0.3.0) (**significant API changes**).
  - `xpocketmp/raklib` has been updated from 0.12.7 to [0.14.2](https://github.com/pmmp/RakLib/releases/tag/0.14.2) (**significant API changes**, see also [0.13.0](https://github.com/pmmp/RakLib/releases/tag/0.13.0)).
  - `xpocketmp/raklib-ipc` has been added at [0.1.0](https://github.com/pmmp/RakLibIpc/releases/tag/0.1.0) (components extracted from RakLib).
  - `xpocketmp/snooze` has been updated from 0.1.0 to [0.3.0](https://github.com/pmmp/Snooze/releases/tag/0.3.0) (**minor API changes**, see also [0.2.0](https://github.com/pmmp/Snooze/releases/tag/0.2.0)).
  - `xpocketmp/spl` has been dropped.
...
- The following submodules have been removed:
  - `resources/vanilla`: BedrockData is now included via Composer dependency [`xpocketmp/bedrock-data`](https://packagist.org/packages/xpocketmp/bedrock-data).
  - `resources/locale`: Language files are now included via Composer dependency [`xpocketmp/locale-data`](https://packagist.org/packages/xpocketmp/locale-data).
...
### Tools
Some new scripts have been added in the `tools/` directory of the repository. These scripts may use the xpocketmp-MP core library, but are intended to be run standalone.
...
### Configuration
- World presets can now be provided as a `preset` key in `xpocketmp.yml`, instead of putting them in the `generator` key.
- The `worlds` config no longer supports attaching the generator settings to the `generator` key (use the `preset` key instead).
- Using an unknown generator in `server.properties` or `xpocketmp.yml` will now cause a failure to generate the world.
- Using invalid/incorrect world generator options (presets) in `server.properties` or `xpocketmp.yml` will now cause a failure to generate the world.
- The following new options have been added to `xpocketmp.yml`:
...
- A new setting has been added to `xpocketmp.yml`: `network.enable-encryption` which is `true` by default.
...
### Internals
- The `xpocketmp` subdirectory has been removed from `src`. [PSR-4 autoloading is now used thanks to Composer](https://github.com/pmmp/xpocketmp-MP/blob/4.0.0/composer.json#L63).
...
- Everything in the `xpocketmp\metadata` namespace and related implementations have been removed.
...
- The `xpocketmp\tile` namespace has been relocated to `xpocketmp\block\tile`.
...
#### Effect
- All `Effect` related classes have been moved to the `xpocketmp\entity\effect` namespace.
...
- The boundaries between MCPE effect IDs and xpocketmp-MP internals are now more clear.
  - ID handling is moved to `xpocketmp\data\bedrock\EffectIdMap`.
  - All effect ID constants have been removed from `Effect`. `xpocketmp\data\bedrock\EffectIds` if you still need legacy effect IDs for some reason.
...
- All network metadata related constants have been removed from the `Entity` class and moved to the protocol layer. It is intended to remove network metadata from the API entirely, but this has not yet been completed.
  - `Entity::DATA_FLAG_*` constants have been moved to `xpocketmp\network\mcpe\protocol\types\entity\EntityMetadataFlags`.
  - `Entity::DATA_TYPE_*` constants have been moved to `xpocketmp\network\mcpe\protocol\types\entity\EntityMetadataTypes`.
  - `Entity::DATA_*` constants have been moved to `xpocketmp\network\mcpe\protocol\types\entity\EntityMetadataProperties`.
- `DataPropertyManager` has been moved to the `xpocketmp\network\mcpe\protocol\types\entity` namespace, and as such isn't considered part of the API anymore.
...
- The internal event processing system no longer depends on `Listener` objects. Arbitrary closures can now be used, provided that they satisfy the standard requirements to be a handler.
  ...
  - The following classes have been removed:
    - `xpocketmp\plugin\EventExecutor`
    - `xpocketmp\plugin\MethodEventExecutor`
...
- The following classes have been moved:
  - `xpocketmp\plugin\RegisteredListener` -> `xpocketmp\event\RegisteredListener`

### Inventory
- All crafting and recipe related classes have been moved to the `xpocketmp\crafting` namespace.
- The following classes have been added:
  - `CallbackInventoryChangeListener`
  - `CreativeInventory`: contains the creative functionality previously embedded in `xpocketmp\item\Item`, see Item changes for details
...
- The following classes have been renamed / moved:
  - `ContainerInventory` -> `xpocketmp\block\inventory\BlockInventory`
- The following classes have been moved to the `xpocketmp\block\inventory` namespace:
  - `AnvilInventory`
  - `ChestInventory`
...
- The following methods have been moved to `xpocketmp\inventory\CreativeInventory`:
...
- The boundaries between MCPE enchantment IDs and xpocketmp-MP internals are now more clear.
  - ID handling is moved to `xpocketmp\data\bedrock\EnchantmentIdMap` singleton.
  - All enchantment ID constants have been removed from `Enchantment`. `xpocketmp\data\bedrock\EnchantmentIds` if you still need legacy effect IDs for some reason.
...
- The following new permission nodes have been introduced:
  - `xpocketmp.group.everyone`: granted to everyone by default
  - `xpocketmp.group.operator`: granted to operator players and the console
...
- Permissions internally no longer have default values. Instead, they are now assigned as a child of one of the `xpocketmp.group` permissions:
  - `true`: add as child to `xpocketmp.group.everyone` with value `true`
  - `false`: do not add to any permission
  - `op`: add as child to `xpocketmp.group.operator` with value `true`
  - `notop`: add as child to `xpocketmp.group.everyone` with value `true`, and to `xpocketmp.group.operator` with value `false`
...
- The following API methods have been added:
  ...
  - `Permissible->setBasePermission()` - used for assigning root permissions like `xpocketmp.group.operator`; plugins usually shouldn't use this
...
- The following API methods have changes:
  ...
  - `Permission->__construct()` no longer accepts a `$defaultValue` parameter (see notes above about defaults refactor).you should add your permission as a child of `xpocketmp.group.everyone` or `xpocketmp.group.operator` instead).
- The following classes have been removed:
  - `ServerOperator`

### Player
- The following classes have been added/moved to the new `xpocketmp\player` namespace:
  - `Achievement`
  - `GameMode`
  - `IPlayer`
  - `OfflinePlayer`
...
- `xpocketmp\Collectable` has been removed, and is no longer extended by `AsyncTask`.
- The following hooks have been added:
...
- New chat broadcasting APIs have been implemented, which don't depend on the permission system.
  ...
  - Giving `Player` any `xpocketmp.broadcast.*` permissions will cause them to automatically subscribe to the corresponding broadcast channel (and removing them will unsubscribe it).
...
- The following API methods have been removed:
  ...
  - `Server->getGamemodeString()` - replaced by `xpocketmp\player\GameMode->getTranslationKey()`
  - `Server->getGamemodeName()` - replaced by `xpocketmp\player\GameMode->name()`
...
- All references to `Level` in the context of "world" have been changed to `World`.
  - The `xpocketmp\level` namespace has been renamed to `xpocketmp\world`
...
- The following classes have been renamed:
  - `xpocketmp\world\utils\SubChunkIteratorManager` -> `xpocketmp\world\utils\SubChunkExplorer`
...
- `Location` has been moved to `xpocketmp\entity\Location`.

#### Particles
- `DestroyBlockParticle` has been renamed to `BlockBreakParticle` for consistency.
- `DustParticle->__construct()` now accepts a `xpocketmp\color\Color` object instead of `r, g, b, a`.
- `xpocketmp\world\particle\Particle` no longer extends `xpocketmp\math\Vector3`, and has been converted to an interface.
- Added the following `Particle` classes:
  - `DragonEggTeleportParticle`
  - `PunchBlockParticle`

#### Sounds
- `xpocketmp\world\sound\Sound` no longer extends `xpocketmp\math\Vector3`, and has been converted to an interface.
- `Sound->encode()` now accepts `xpocketmp\math\Vector3`.
...
### Utils
- The `Color` class was removed. It's now found as `xpocketmp\color\Color` in the [`xpocketmp/color`](https://github.com/pmmp/Color) package.
...
- Fixed all chunks containing furnaces being treated as corrupted in worlds older than 2017.
  - This was caused by a strict corruption check detecting bad data created by a bug in xpocketmp-MP that was fixed in 2017.
