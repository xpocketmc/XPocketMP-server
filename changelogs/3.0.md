...
This release is the first to use the new merged versioning system. The xpocketmp-MP version and the API version are now one and the same.
...
## Core
### General
- This release introduces a new dependency library called `Snooze`, which xpocketmp-MP utilizes for managing notifications between threads. This library is currently utilized by command reading, RCON and RakLib, allowing faster response times to commands, reduced network latency, and better performance due to lack of need for polling.
- Fixed race condition causing `ServerKiller` to sometimes kill the server for no reason on shutdown.

### Dependency libraries
Requires the following libraries:
- [`xpocketmp/raklib` `^0.12.0`](https://github.com/pmmp/RakLib/releases/0.12.0)
- [`xpocketmp/spl` `^0.3.0`](https://github.com/pmmp/SPL/releases/0.3.0)
- [`xpocketmp/binaryutils` `^0.1.0`](https://github.com/pmmp/BinaryUtils/releases/0.1.0)
- [`xpocketmp/nbt` `^0.2.0`](https://github.com/pmmp/NBT/releases/0.2.0)
- [`xpocketmp/math` `^0.2.0`](https://github.com/pmmp/Math/releases/0.2.0)
- [`xpocketmp/snooze` `^0.1.0`](https://github.com/pmmp/Snooze/releases/0.1.0)
...
### Levels
- Seed handling is now always consistent between `xpocketmp.yml` and `server.properties`.
...
### Plugins
- Plugin data is now stored under `<data dir>/plugin_data` by default on new installations, instead of in `<plugins dir>`. This behaviour does not affect existing installations by default - if you want to enable it, set `plugins.legacy-data-dir` to `false` in `xpocketmp.yml`.
...
The way that the API versioning is done has changed. Now the API version is synonymous with the xpocketmp-MP version.
...
### General
- `Server->getCodename()` and `\xpocketmp\CODENAME` have been removed.
- Added API method `Server->getTickSleeper()`, which returns a `\xpocketmp\snooze\SleeperHandler` instance.
...
### Level
- `Biome` classes have been moved to `xpocketmp\level\biome` namespace. This is in preparation for future work on Levels which requires biome information (such as weather conditions).
