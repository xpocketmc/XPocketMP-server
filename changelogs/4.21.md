...
## Configuration
- Added the following new `xpocketmp.yml` configuration options:
...
- `RuntimeBlockMapping` no longer keeps all block palette NBT data in memory.
  ...
  - NBT will be lazy-loaded into memory and cached if `getBedrockKnownStates()` is called. However, this is not used by xpocketmp-MP under normal circumstances.
