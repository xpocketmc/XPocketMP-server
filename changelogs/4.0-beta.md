...
- The boundaries between MCPE enchantment IDs and xpocketmp-MP internals are now more clear.
  - ID handling is moved to `xpocketmp\data\bedrock\EnchantmentIdMap`.
...
### Player
- The following classes have moved to the new `xpocketmp\player` namespace:
  - `Achievement`
  - `GameMode`
  - `IPlayer`
  - `OfflinePlayer`
...
- The following API methods have been removed:
  ...
  - `enablePlugin()`
  - `disablePlugin()`
  - `getGamemodeString()` - replaced by `xpocketmp\player\GameMode->getTranslationKey()`
  - `getGamemodeName()` - replaced by `xpocketmp\player\GameMode->name()`
...
- `Sound->encode()` now accepts `?\xpocketmp\math\Vector3`. `NULL` may be passed for sounds which are global.
...
- The following API methods have been removed:
  ...
  - `Chunk->removeEntity()`
- The following classes have been added:
  - `xpocketmp\inventory\transaction\TransactionBuilderInventory`: facilitates building `InventoryTransaction`s using standard `Inventory` API methods
...
### Block
- Added the following classes:
  - `xpocketmp\block\utils\LeverFacing`
- Added the following API methods:
  - `xpocketmp\block\Lever->isActivated()`
  - `xpocketmp\block\Lever->setActivated()`
  - `xpocketmp\block\Lever->getFacing()`
  - `xpocketmp\block\Lever->setFacing()`
...
- The `worlds` config in `xpocketmp.yml` no longer supports attaching the generator settings to the `generator` key (use the `preset` key instead).
...
- Fixed stats reporting checking for a nonexistent `xpocketmp.yml` setting.
...
- Now uses [`xpocketmp/bedrock-protocol@58c53a259e819a076bf8fe875d2a012da7d19d65`](https://github.com/pmmp/BedrockProtocol/tree/58c53a259e819a076bf8fe875d2a012da7d19d65). This version features significant changes, including:
...
- [`xpocketmp/math` 0.4.0](https://github.com/pmmp/Math/releases/tag/0.4.0) is now used. Please see its release notes for changes.
...
- `resources/vanilla` submodule has been removed. BedrockData is now included via Composer dependency [`xpocketmp/bedrock-data`](https://packagist.org/packages/xpocketmp/bedrock-data).
- `xpocketmp/spl` Composer dependency has been dropped.
- The following Composer dependency versions are now required:
  - [`xpocketmp/bedrock-protocol` v5.0.0](https://github.com/pmmp/BedrockProtocol/tree/5.0.0+bedrock-1.17.40) features substantial changes to its API compared to 3.0.1, which was used in 4.0.0-BETA8. Please see its [release notes](https://github.com/pmmp/BedrockData/releases/tag/5.0.0+bedrock-1.17.40).
  - [`xpocketmp/log` v0.4.0](https://github.com/pmmp/Log/tree/0.4.0) removes the `LoggerAttachment` interface and replaces logger attachment objects with closures.
  - [`xpocketmp/log-pthreads` v0.4.0](https://github.com/pmmp/LogPthreads/tree/0.4.0)
  - [`xpocketmp/classloader` v0.2.0](https://github.com/pmmp/ClassLoader/tree/0.2.0)
...
- `Promise` API has changed:
  - Promise-related classes have been moved to `xpocketmp\promise` namespace.
...
## General
- `resources/locale` submodule has been removed. Language files are now included via Composer dependency [`xpocketmp/locale-data`](https://packagist.org/packages/xpocketmp/locale-data).
...
- Fixed ungenerated chunks saved on disk by old versions of xpocketmp-MP (before 3.0.0) being treated as corrupted during world conversion (they are now ignored instead).
