...
### `xpocketmp\player`
- The following new API methods have been added:
  - `public Player->getCreativeInventory() : xpocketmp\inventory\CreativeInventory`
  - `public Player->setCreativeInventory(xpocketmp\inventory\CreativeInventory $inventory) : void`
...
## Internals
- Uncaught exceptions and fatal errors in `AsyncTask`, threads extending `xpocketmp\thread\Thread`, and `xpocketmp\thread\Worker` are now recorded in crashdumps, making it significantly easier to debug errors in these areas.
- JWT signature DER <-> raw conversions are now handled in-house using code in `JwtUtils`
  ...
  - `fgrosse/phpasn1` is no longer required. This package was abandoned by its author and only used by xpocketmp-MP for this one purpose.
...
- Fixed `xpocketmp\thread\Thread` and `xpocketmp\thread\Worker` instances not logging any information when they crash.
...
## Included releases
- [4.23.5](https://github.com/pmmp/xpocketmp-MP/blob/4.23.5/changelogs/4.23.md#4235) - Minor bug fixes
...
## Included releases
- [4.23.6](https://github.com/pmmp/xpocketmp-MP/blob/4.23.6/changelogs/4.23.md#4236) - Armor inventory client bug workaround
