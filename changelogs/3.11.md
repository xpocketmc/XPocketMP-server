...
- The core constants `xpocketmp\PATH` and `xpocketmp\RESOURCE_PATH` are now unconditionally available when including the Composer autoloader.
...
- Updated Composer dependencies to get bug fixes in `xpocketmp/nbt` and other libraries.
- Packages `xpocketmp/classloader` and `xpocketmp/log` are now required; these provide classes previously part of `xpocketmp/spl`. This change has no effect on API compatibility.
...
- Introduced a new dependency on `xpocketmp/log-pthreads`, which contains classes separated from `xpocketmp/log`.
- Fixed minimum composer stability preventing any newer version of `xpocketmp/xpocketmp-mp` being installed than 3.3.4 by replacing `daverandom/callback-validator` with [`xpocketmp/callback-validator`](https://github.com/pmmp/CallbackValidator).
