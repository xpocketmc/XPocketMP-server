...
- Fixed plugin detection in crashdumps. Previously `src/xpocketmp` anywhere in the path would cause the error to be considered a core crash, regardless of the preceding path.
...
- `Internet::simpleCurl()` now includes the xpocketmp-MP version in the user-agent string.
...
- The following constants are now autoloaded when loading the Composer autoloader:
  - `xpocketmp\NAME`
  - `xpocketmp\BASE_VERSION`
  - `xpocketmp\IS_DEVELOPMENT_BUILD`
  - `xpocketmp\BUILD_NUMBER`
  - `INT32_MIN`
  - `INT32_MAX`
  - `INT32_MASK`
